...
`AppComponentsResolver` 是一个用�?`unplugin-vue-components` 的自定义组件自动导入解析器�?
它默认扫描当前项目的 `src/components` 顶层入口，并把符合约定的组件注册�?`unplugin-vue-components`，让页面模板可以直接使用通用组件�?

## 1. 为什么这么做

项目里的通用组件通常会放�?`src/components` 下，如果每个页面都手动写 `import`，会有几个问题：

- 组件一多，页面顶部�?`import` 会越来越乱�?- 同一个组件在不同页面反复手写，重复代码很多�?- 组件目录调整后，很多页面都要跟着改导入路径�?
...
- 默认只处�?`src/components`，范围明确，不会把业务组件一起卷进来�?- 支持通过 `componentsDir` 配置组件目录，适配不同项目结构�?- 支持通过 `importBase` 配置导入路径前缀，不强制项目必须使用 `@` 别名�?- 组件按使用自动导入，页面代码更干净�?
...
这里 `dirs: []` 不要删�?
这样做的目的是关�?`unplugin-vue-components` 默认目录扫描，只保留自己的解析规则�?
...
如果 Vite 项目没有配置 `@ -> src` 别名，默认生成的 `@/...` 导入路径会解析失败。此时可以使�?`importBase` 显式指定导入路径前缀�?

### importBase

生成自动导入时使用的路径前缀�?
例如不使�?`@` 别名时，可以配置�?Vite 支持�?`/src/...` 路径�?
...
同一个组件会同时注册�?`PascalCase`、`camelCase` �?`kebab-case`�?
