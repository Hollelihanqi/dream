...
插件会在 Vite `serve` �?`build` 阶段生成一�?Element Plus 主题 CSS�?

- 开发模式：生成完整主题，方便本地调试�?- 构建模式：扫描源码中实际使用�?Element Plus 组件，按需生成主题样式�?- 输出结果：只写入最�?CSS 文件，不额外保留临时 SCSS 文件�?
...
| 参数                       | 类型                         | 默认�?                                                          | 说明                             |
| -------------------------- | ---------------------------- | --------------------------------------------------------------- | -------------------------------- |
| `outputCssPath`            | `string`                     | `src/assets/generated/element-plus-theme.css`                   | 最�?CSS 输出路径，基于项目根目录 |
...
| `colors`                   | `Partial<ThemeColorPalette>` | 内置默认色板                                                    | 覆盖主题�?                       |
...
1. 是否引入了生成后�?CSS 文件�?2. 是否仍然引入�?`element-plus/dist/index.css`�?3. 是否重启了开发服务器�?4. 浏览器是否使用了旧缓存�?

### 2. 构建时报找不�?Element Plus 主题源码

请检查：

1. 是否安装�?`element-plus`�?2. `elementPlusThemeChalkDir` 是否配置正确�?

### 3. 想扩展扫描类�?

可以自定�?`scanFilePattern`�?
...
- 使用方项目需要安�?`element-plus` �?`sass`�?- 修改主题色后如未生效，请重启开发服务器�?- 路径类配置建议使用相对项目根目录的写法�?
